...
const loadComments = async (organizerId) => {
  try {
    ...
    if (commentsResponse && commentsResponse.code === 0 && commentsResponse.data) {
      // 统一 id / parentId
      const rows = commentsResponse.data.map(c => ({
        ...c,
...
const onReplyClick = (e, commentId) => {
  ...
  replyContent.value = '';
  console.log('[OrganizerProfileView] onReplyClick -> replyingTo =', replyingTo.value);
}
...
const submitReply = async (commentId) => {
  ...
  try {
    ...
    console.log('[OrganizerProfileView] Submitting reply to comment:', commentId);
    ...
    const commentData = {
      ...
      toOrganizerId: Number(organizer.value.id), // 设置目标组织者ID
      rating: null, // not set rating
      parentCommentId: Number(normalizeId(commentId)) // 关键：用规范化后的 ID
    }
    
    console.log('[OrganizerProfileView] Reply comment data:', commentData)
    
    const response = await commentApi.createComment(commentData)
    
    if (response && response.code === 0) {
      console.log('[OrganizerProfileView] Reply submitted successfully');
      // 重新加载评论列表
      await loadComments(organizer.value.id);
      // 重置回复状态
    ...
    console.error('[OrganizerProfileView] Error submitting reply:', error);
...
const cancelReply = () => {
  replyingTo.value = null;
  replyContent.value = '';
  console.log('[OrganizerProfileView] Reply cancelled');
}
