...
const loadComments = async (eventId) => {
  try {
    ...
    if (commentsResponse && commentsResponse.code === 0 && commentsResponse.data) {
      ...
      console.log('[EventDetailView] Loaded comments:', comments.value)
      // Debug: Check each comment's fromUserType
      comments.value.forEach((comment, index) => {
        console.log(`[EventDetailView] Comment ${index}: fromUserName=${comment.fromUserName}, fromUserType=${comment.fromUserType}`)
      })
    } else {
